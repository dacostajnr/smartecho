  ...
  .init(function () {
    ...
    var onRequest = function (pdu, cb) {
      setTimeout(function () {
        ...
        //   return
        // }
        if ((start > (memhold.length/2)-1) || start + quantity > (memhold.length/2)) {
          let buf = Buffer.allocUnsafe(2)
          buf.writeUInt8(0x81, 0)
          buf.writeUInt8(0x02, 1)
          cb(buf)
        ...
        for (var totalBitCount = start; totalBitCount < start + quantity; totalBitCount += 1) {
          var buf = ( memhold.readUInt16BE(totalBitCount*2) > 0 ) ?1:0;// mem.readUInt8(Math.floor(totalBitCount / 8))
          var mask = 1;// << (totalBitCount % 8)
          //console.log(totalBitCount)
          if (buf & mask) {
