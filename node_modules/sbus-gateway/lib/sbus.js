...
var sbus = {
    ...
    csumCode: function (csumString,ip=null,port=null) {
        ...
        if ((ip!=null) && (port!=null)){
            ...
            sbus.sendFinalMessage(data,ip,port);
        }
    ...
    sendFinalMessage:function(data,ip,port){
        try{
            console.log(`I am sending the message now to ${ip}:${port}`);
    ...
    lighting: function (subnetid, deviceid, channel, level, time,ip,port) {
       ...
       sbus.csumCode(string,ip,port);
        ...
        ReadMacAddress:function(subnetid,deviceid,port,ip){  //subnetid,deviceid,ip,port
        var lengthdata = "\x0B";

        var deviceSubnetID="\xBB";
        var senderDeviceID = "\xBB";
        var senderDeviceType="\xDD\xDD";

        var operationCode = "\xF0\x03";
        var targetSubnetID =String.fromCharCode(subnetid);//"\x01";
        var targetDeviceID=String.fromCharCode(deviceid); //"\x15";

        var string = lengthdata + deviceSubnetID + senderDeviceID +senderDeviceType + operationCode +targetSubnetID + targetDeviceID;
        sbus.csumCode(string,ip,port);
        console.log(`Sending command [ReadMac address] ==> to ${ip}:${port}`);
    },
